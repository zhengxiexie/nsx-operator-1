...
func TestCreateDefaultSubnetSet(t *testing.T) {
	tests := []struct {
		...
		{
			name:               "Error case - failed to create SubnetSet",
			namespace:          "test-ns",
			defaultSubnetSize:  24,
			existingResources:  []client.Object{},
			expectedError:      true,
			...
			setupMocks: func(r *NamespaceReconciler) *gomonkey.Patches {
				return gomonkey.ApplyMethod(reflect.TypeOf(r.Client), "Create",
					func(_ client.Client, _ context.Context, _ client.Object, _ ...client.CreateOption) error {
						return fmt.Errorf("failed to create SubnetSet")
					})
			},
		},
	}
...
func TestDeleteDefaultSubnetSet(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "Success case - delete existing SubnetSets",
			namespace: "test-ns",
			existingResources: []client.Object{
				&v1alpha1.SubnetSet{
					ObjectMeta: metav1.ObjectMeta{
		...
		{
			name:      "Error case - failed to delete SubnetSet",
			namespace: "test-ns",
			existingResources: []client.Object{
				&v1alpha1.SubnetSet{
					ObjectMeta: metav1.ObjectMeta{
			...
			expectedError: true,
			setupMocks: func(r *NamespaceReconciler) *gomonkey.Patches {
				return gomonkey.ApplyMethod(reflect.TypeOf(r.Client), "Delete",
					func(_ client.Client, _ context.Context, _ client.Object, _ ...client.DeleteOption) error {
						return fmt.Errorf("failed to delete SubnetSet")
					})
			},
		},
	}
...
func TestCreateNetworkInfoCR(t *testing.T) {
	// Test cases
	tests := []struct {
		name              string
		namespace         string
		...
		{
			name:              "Success case - create new NetworkInfo CR",
			namespace:         "test-ns",
			existingResources: []client.Object{},
			expectedError:     false,
		},
		{
			name:      "Success case - NetworkInfo CR already exists",
			namespace: "test-ns",
			existingResources: []client.Object{
		...
		{
			name:              "Error case - failed to create NetworkInfo CR",
			namespace:         "test-ns",
			existingResources: []client.Object{},
			expectedError:     true,
			expectedErrString: "already exists",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// For the error case, use a custom client that returns an error on Create
			if tt.expectedError {
				customClient := fake.NewClientBuilder().WithScheme(r.Scheme).WithObjects(tt.existingResources...).Build()
			...
			// Call the function being tested
			networkInfo, err := r.createNetworkInfoCR(context.Background(), ns, tt.namespace)
			...
			if tt.expectedError {
				...
				if len(tt.existingResources) > 0 {
					// If NetworkInfo already exists, we should get the existing one
					assert.Equal(t, "test-ns", networkInfo.Name)
				} else {
					// If NetworkInfo is created, it should have the namespace name
					assert.Equal(t, tt.namespace, networkInfo.Name)
				}
				assert.Equal(t, tt.namespace, networkInfo.Namespace)
				// Check that VPCs is initialized as an empty slice
				assert.NotNil(t, networkInfo.VPCs)
				assert.Equal(t, 0, len(networkInfo.VPCs))
			}
		})
	}
}
