...
func (service *SecurityPolicyService) securityPolicyEqual(sp1 *model.SecurityPolicy, sp2 *model.SecurityPolicy) *model.SecurityPolicy {
...
func (service *SecurityPolicyService) rulesEqual(existingRules []model.Rule, rules []model.Rule) ([]model.Rule, []model.Rule) {
	...
	for ruleId, rule := range rulesMap {
		if existingRule, ok := existingRulesMap[ruleId]; ok {
			if service.ruleEqualDetail(existingRule, rule) {
				continue
			}
		}
...
func (service *SecurityPolicyService) ruleEqualDetail(existingRule model.Rule, rule model.Rule) bool {
...
func (service *SecurityPolicyService) groupsEqual(existingGroups []model.Group, groups []model.Group) ([]model.Group, []model.Group) {
	...
	for groupId, group := range groupsMap {
		if existingGroup, ok := existingGroupsMap[groupId]; ok {
			if service.groupEqualDetail(existingGroup, group) {
				continue
			}
		}
...
func (service *SecurityPolicyService) groupEqualDetail(existingGroup model.Group, group model.Group) bool {
